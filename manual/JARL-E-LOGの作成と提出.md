---
title: JARL E-LOG R1.0/R2.1の作成
---
## 概要

JARL E-LOG R1.0又はR2.1を作成します。V2.9.2.4よりR1.0とR2.1のウインドウを統合しました。切替はウインドウ上部のタブで切り替えます。    
...
## 作成方法
①コンテスト後は「ファイル」－「JARL E-Logの作成]メニューをクリックします。  
![E-LOG 2.1の作成](https://raw.githubusercontent.com/nextzlog/use.zlog.org/master/images/jarl-elog-menu.png?raw=true)

②入力ダイアログに必要事項を入力します。  
　参加部門以外のバンドでのQSOがある場合は、右側の「スコア調整」欄で参加部門以外のバンドをチェックOFFとして下さい。  
　参加部門以外のバンドのQSOのログはログチェックに使用すると思われますので削除する必要はありません。  
![必要事項の入力](https://raw.githubusercontent.com/nextzlog/use.zlog.org/master/images/jarl-elog.png?raw=true)
...
## 提出方法
### JARL主催コンテスト
#### Webアップロードでの提出(v2.9.4.4～)
ウィンドウ下部の「Webアップロード」ボタンをクリックすることで、JARLの電子ログアップロードページに自動的にログを貼り付けます。後は画面の指示に従い、「貼り付けたテキストをアップロードする」ボタンをクリック、次の確認画面で「送信する」ボタンを押すことでログの提出が完了します。  
#### 電子メールでの提出
