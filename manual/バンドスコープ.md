...
| --- | --- | --- |
|1|交信済み|交信済みの局|
|2|未交信マルチ|未交信マルチの局（他バンドで交信済みやLookup Serverでマルチが判明している場合）|
|3|交信済みマルチ|交信済みマルチかつ未交信の局（急いでゲットする必要はない）|
|4|その他|不明の局|

### スポットの信頼度

さらにV2.9.6.0よりスポット信頼度という概念を導入しています。 
信頼度に合わせて表示色を設定できます。  
* AR-Clusterでは"CT1BOH品質タグ"というフィールドがあり、?,V,Q,BでSPOTの信頼度を表しています。
* zLog独自に、スポッター別の設定又はスポット報告回数で信頼度を設定します。

|スポットの信頼性|条件|
|---|---|
|高(High)|高信頼スポッタ― and 品質タグ"V","Q","?","" and QTHLookup照会成功<br>（３つ以上のスポッターから報告ありでVかつ高信頼スポッター扱い）|
|中(Middle)|低信頼スポッタ― and 品質タグ"V","Q","?","" and QTHLookup照会成功|
|低(Low)|品質タグ"B" or QTHLookup照会失敗（即ちHigh,Middle以外）|

※自分で設置したSKIMMERは、高信頼スポッターに入れると良いです。  

## 設定

### 保持時間（有効期間）の設定

「各種設定」－「運用設定」メニューをクリックし、「チェッカー」タブを開きます。  
...
「各種設定」－「運用設定」メニューをクリックし、「バンドスコープ」タブを開きます。  
①バンドごとにウインドウが表示されるので使用するバンドにチェックを入れます。  
②お好みで表示色を設定します。  
③お好みでバンドスコープオプションを設定します。  
...
|2|JAを表示|チェックONで国内局を表示します。|
|3|DXを表示|チェックONで国外局を表示します。|
|4|マルチ照会を行う|チェックONで他バンドの交信よりマルチを表示します。|
|5|Lookup Serverを使用|チェックONでマルチ判定用に外部サーバーを使用します。|
|6|レジューム機能を使う|チェックONでzLog終了時にバンドスコープ内スポットを保存し、次のzLog起動時に読み込み復元します。有効期間はzLog終了から30分以内です。（V2.8.7.2以降）|
|7|周波数より推定したモードを使う|チェックONで周波数をバンドプランに照合し、モードを推定します。|
|8|　LSB/USBモード誤りを抑制|チェックONでモード設定コマンドを送信します。OFFの場合はリグが記憶している設定を使います。|
|9|　周波数ずれを抑制|チェックONで周波数ずれを抑制します。（YAESU機で発生するサイドバンド分ずれる現象の対策です）|
|10|SPOTにQSYする直前の周波数を記憶|チェックONでスポットにQSYする直前の周波数をLast freq.に記憶します。（戻ってこれるようにする）<br>1Radioの場合のみ使用可能|

### 表示色の設定２

「各種設定」－「運用設定」メニューをクリックし、「バンドスコープ２」タブを開きます。  
