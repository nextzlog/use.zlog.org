...
通常のCQ用メッセージは #1 に登録します。2BSIQ時は反対側で使用するCQメッセージを変更することができます。  
「ハードウェア設定」－「運用スタイル」タブで設定します。  
![2BSIQのCQメッセージ](../images/2bsiq_cqmessage.png)  

メッセージ設定のヒントとしては、RIG-AでCQをかけ呼んできた相手局をピックアップしますが、ここで反対側(RIG-B)のCQが長いと送信が終わるまでRIG-A側で呼んできた局を待たせることになります。逆に短いと両方が受信になってしまいます。受信と送信が均等になるようなイメージで設定するのが良いように思います。  
