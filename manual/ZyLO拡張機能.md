...
zLogには、横林OMを中心に、JA1ZLOで開発された[オリジナル版](https://zlog.org)と、JR8PPG局らを中心に、オープンソースで開発された[令和版](https://github.com/jr8ppg/zLog)があります。
令和版には、古いPC用の32bit(x86)版と、新しいPC用の64bit(x64)版がありますが、64bit版でプラグイン機能を利用できます。
厳密には、v2.8以降です。
細かいことはともかく、[こちらのページ](https://zylo.pafelog.net/builds)の一番上にある最新版のうち、x64版をダウンロードすれば大丈夫です。
...
zLogで利用可能なプラグインのリストは、[こちらのページ](https://zylo.pafelog.net/market)か、後述するzLog本体のプラグインマネージャから確認できます。
掲載されているプラグインは、いずれもzLogユーザーが自作し、ZyLO開発チームが依頼を受けて登録したプラグインです。

### プラグインが見つからない

[こちらのページ](https://zylo.pafelog.net/market)には掲載されているのに、zLogのプラグインマネージャに表示されないプラグインがあります。
