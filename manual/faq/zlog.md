...
* [FAQ-18 マルチOPモードで使用したあと、QSY Indicator ウインドウが消せなくなってしまったのですが？](#faq-18-%E3%83%9E%E3%83%AB%E3%83%81op%E3%83%A2%E3%83%BC%E3%83%89%E3%81%A7%E4%BD%BF%E7%94%A8%E3%81%97%E3%81%9F%E3%81%82%E3%81%A8qsy-indicator-%E3%82%A6%E3%82%A4%E3%83%B3%E3%83%89%E3%82%A6%E3%81%8C%E6%B6%88%E3%81%9B%E3%81%AA%E3%81%8F%E3%81%AA%E3%81%A3%E3%81%A6%E3%81%97%E3%81%BE%E3%81%A3%E3%81%9F%E3%81%AE%E3%81%A7%E3%81%99%E3%81%8C)
...
zLogではサマリーシート・ログシートの直接の印刷はサポートしていません。今すぐお近くのハムショップに行って「コンテスト用紙」を買って下さいといいたい所ですが、既に販売は終了しています。[JARLコンテスト委員会が提供するPDF](https://www.jarl.org/Japanese/1_Tanoshimo/1-1_Contest/paper_submission.html)を使って手書きするか、[有志のかたが作成したExcelファイル](JARL_logsheet.zip)を活用するしかありません。このファイルにzLogから出力したログの内容を貼り付けて印刷しましょう。  
...
なお、省略形を適用するか否かは、[置換マクロ($R, $X など)](https://use.zlog.org/manual/CW%E3%82%AD%E3%83%BC%E3%82%A4%E3%83%B3%E3%82%B0#%E7%BD%AE%E6%8F%9B%E3%83%9E%E3%82%AF%E3%83%AD) の大文字・小文字で区別されます。大文字で適用・小文字で不適用となります。たとえば、$R$x とするとRSTには省略形が適用され"5NN"などとなり、ナンバー(NR)には適用されず設定された文字がそのまま送信されることになります。
...
ICOMのCI-Vは一部コマンドで、コマンド受信後にOKの応答(いわゆるACK応答)を行うのですが、zLogがOKの応答を受信できないときに表示しています。動作に支障は無いので問題ありません。気にしないで下さい。(2026年2月現在 V3.0.0.0 以後のバージョンでは解消されました。)    
...
zLogから出力した JARL E-log 型式や CSV型式のファイル、あるいは "zlog.ini" ファイルを編集するため、Windowsの「メモ帳」などのアプリで開いたら文字化けすることがあるのですが、どうしたらいいでしょうか？  
...
対処方法としては、アプリ側がシフトJISに対応している場合は漢字コードを指定してファイルを開くことです。たとえば Windows11 標準の「メモ帳」では、ファイルを開くダイアログの右下に「エンコード」という項目があり、"ANSI" を選択して開くことで正しく表示されます。(ダブルクリックで開いてしまうと、メモ帳を一旦閉じなければその選択が反映されません。また、ダブルクリックで開くと、漢字コードの判定を誤る場合があります。）漢字コードを変換して保存したい場合は、"名前をつけて保存"から「エンコード」で他のコード(UTF-8など)を選択してから保存します。  
...
ほかの選択肢として、文字情報技術促進協会が提供している [IPAフォント](https://moji.or.jp/ipafont/ipa00303/) を使う方法があります。Consolas は半角文字のみ対応ですが、IPAフォントは全角文字にも対応しています。(ただし、全角文字のゼロには斜線が入りません。）
...
(2025.7.25 追記) Ver.2.9.5.0 より、このオプションがCFG選択ウインドウでon/offできるようになりました。また、デフォルトをon(一時変更可)としました。
...
(2022/7/18 時点) zlistwでZLOファイルの編集・更新を行うと、日時がUTCになってしまうなど、意図しない結果になるケースが報告されています。最新バージョンのzLog令和版では、ログの修正や分析など、zlistwでできるほとんどのことが本体内部で行えるようになっていますので、今後は zlistw は使用しないようお願いします。また、今後はファイル保存形式のデフォルトが *.zlox ファイルとなりますが、zlistwはこれに対応していませんのでご注意願います。

## FAQ-18 マルチOPモードで使用したあと、QSY Indicator ウインドウが消せなくなってしまったのですが？

### 質問

マルチOPモードにしたときに QSY Indicator ウインドウが表示されましたが、ウインドウを消してもまたすぐに出てきてしまいます。どうしたら消せますか？

### 答え

「各種設定」メニューから「運用設定」を開き、「コンテストの設定」(V3以後・V2.9では「各種設定」) で "QSYアシスト" を「なし」にしてください。 
