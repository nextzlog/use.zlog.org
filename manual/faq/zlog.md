...
9. [FAQ-9 ICOMのリグ使用時、ステータスバーに赤い文字が出る](#faq-9-icomのリグ使用時ステータスバーに赤い文字が出る)
10. [FAQ-10 SPCファイルを作成する方法は？](#faq-10-spcファイルを作成する方法は)
11. [FAQ-11 zLog を移動運用時や通常交信でも使うには？](#faq-11-zLogを移動運用時や通常交信でも使うには)
...
制限事項としては、2025年4月現在の最新バージョン(Ver.2.9.3.9)までの全てのバージョンで、送信時のRS(T)レポートを59(9)以外に変更できない、という問題があります。もし変更が必要な場合は、いったんMEMO欄などにその旨を記入しておき、後にcsv型式などでログを出力した際に手動で修正する必要があります。（なお、この制限は今後のバージョンで改善される予定です。）
